  ...
  return (
    <main className="min-h-screen px-8 pt-24 pb-16">
      <nav className="fixed left-8 top-8 z-20 bg-cream/80 px-3 py-2 text-[10px] uppercase tracking-[0.22em] text-ink-light backdrop-blur-sm">
        ...
        <span className="mx-2 text-ink-faint">/</span>
        <span className="font-serif italic text-dust-blue">Books {year}</span>
      </nav>

      <section className="mx-auto max-w-5xl">
        <h1 className="text-4xl uppercase tracking-[0.16em] text-ink">
          Books / {year}
        </h1>

        <div className="mt-16">
          {books.map((book) => (
            <article key={book.slug} className="border-b border-ink-faint/30 py-8">
              ...
              <div className="mt-4 flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <h2 className="text-xl uppercase tracking-[0.12em] text-ink">
                  {book.title}
                </h2>
              ...
              {book.hasBody && (
                <div className="mt-5 flex justify-end">
                  <Link
                    ...
                    className="text-[10px] uppercase tracking-[0.32em] text-ink-light transition-colors hover:text-dust-blue"
                  >
                    See More →
                  </Link>
                </div>
        ...
        <div className="mt-12 flex justify-between text-[10px] uppercase tracking-[0.32em] text-ink-light">
          <Link href={`/fun/books/${previousYear}`} className="hover:text-dust-blue">
            ← {previousYear}
          </Link>

          {nextYear && (
            <Link href={`/fun/books/${nextYear}`} className="hover:text-dust-blue">
              {nextYear} →
            </Link>
          )}
        </div>
